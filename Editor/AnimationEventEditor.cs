...
public class AnimationEventEditor : EditorWindow
{
    ...
    private readonly Dictionary<AnimationEventItem, IReadOnlyDictionary<Type, IReadOnlyList<MethodInfo>>> _lookups = new();

    private int _selectedFrameIndex = -1;

    private void OnGUI()
    {
        Animator tmpAnimator = EditorGUILayout.ObjectField("Animator Object", _sourceAnimator, typeof(Animator), true) as Animator;
        if (tmpAnimator == null)
        {
    ...
    private AnimationClip DrawAndFilterAnimationClips()
    {
        ...
        {
            selectedIndex = EditorGUILayout.Popup("Animation", animationClips.IndexOf(_currentClip), animationClips.Select(clip => clip.name).ToArray());
        }
        else
        {
    ...
    {
        if (_baseClipEvents.SequenceEqual(_state.Select(item => item.animationEvent), AnimationEventEqualityComparer.Instance))
        {
        ...
        if (!_state.SequenceEqual(_state.OrderBy(item => item.animationEvent, AnimationEventEqualityComparer.Instance)))
        {
    ...
    private void RenderAnimationEventItem(AnimationEventItem item, int frame, float frameRate)
    {
        ...
        try
        {
            if (1 <= index && Math.Abs(_state[index - 1].animationEvent.time - item.animationEvent.time) < 0.001f && GUILayout.Button("Move Up"))
            {
                _state.RemoveAt(index);
                _state.Insert(index - 1, item);
            }

            if (index < _state.Count - 1 && Math.Abs(_state[index + 1].animationEvent.time - item.animationEvent.time) < 0.001f && GUILayout.Button("Move Down"))
            {
                _state.RemoveAt(index);
                _state.Insert(index + 1, item);
            }

            if (0 <= index && index < _baseClipEvents.Count && !AnimationEventEqualityComparer.Instance.Equals(item.animationEvent, _baseClipEvents[index]) && GUILayout.Button("Reset"))
            {
    ...
    private void TryPopulateTypeAndMethod(AnimationEventItem item, IReadOnlyDictionary<Type, IReadOnlyList<MethodInfo>> lookup)
    {
    ...
    private bool SelectMethods(AnimationEventItem item, IReadOnlyDictionary<Type, IReadOnlyList<MethodInfo>> lookup)
    {
        ...
        int selectedMethodIndex = EditorGUILayout.Popup("MethodName", methods.ToList().IndexOf(item.selectedMethod), methods.Select(method => method.Name).ToArray());
    ...
    private IReadOnlyDictionary<Type, IReadOnlyList<MethodInfo>> FilterLookup(AnimationEventItem item)
    {
        ...
        if (!_restrictedMode)
        {
            ...
            {
                Dictionary<Type, List<MethodInfo>> filtered = Lookup.ToDictionary(kvp => kvp.Key, kvp => kvp.Value.ToList());
    ...
    private void RefreshAnimationEvents()
    {
        ...
        _referenceCurve = AnimationUtility.GetObjectReferenceCurve(_currentClip, EditorCurveBinding.PPtrCurve("", typeof(SpriteRenderer), "m_Sprite")).ToList();
        _referenceCurve.Sort((lhs, rhs) =>
        {
