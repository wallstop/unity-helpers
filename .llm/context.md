...
19. **Run markdown linters after doc changes** — `npm run lint:docs`, `npm run lint:markdown`, and `npm run format:md:check` must pass; `npm run lint:docs` catches missing `./` prefixes on internal links (see rule 27); (see [validate-before-commit](./skills/validate-before-commit.md))
