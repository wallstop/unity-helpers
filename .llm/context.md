...
29. **🚨 Example links in documentation MUST be escaped** — When showing example link syntax in docs (teaching proper format), wrap examples in code blocks or inline backticks so the linter does not parse them as real links; unescaped examples cause false positive lint errors; use fenced code blocks with `markdown` or `text` language specifier for multi-line examples; see [update-documentation](./skills/update-documentation.md#escaping-example-links-in-documentation)
